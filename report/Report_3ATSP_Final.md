...
Khi một nhân viên giao hàng phải phục vụ nhiều điểm trong cùng một chuyến, hệ thống cần giải đồng thời hai bài toán: tìm đường có chi phí thấp nhất giữa từng cặp địa điểm và xác định thứ tự ghé sao cho tổng chi phí của toàn hành trình là nhỏ nhất. Do mạng đường TP.HCM có nhiều đoạn đường một chiều và trọng số phụ thuộc hướng di chuyển, chi phí từ A đến B nói chung khác chi phí từ B đến A. Bài toán thứ tự ghé vì vậy được mô hình hóa dưới dạng **bài toán người bán hàng bất đối xứng** (*Asymmetric Traveling Salesman Problem* — ATSP).

Dự án triển khai ba phương pháp có vai trò bổ sung cho nhau: **Held–Karp** là thuật toán chính xác dùng làm chuẩn tối ưu cho bài toán quy mô nhỏ; **Nearest Neighbor kết hợp 2-opt/Or-opt** là heuristic xác định, ưu tiên thời gian phản hồi; **Simulated Annealing** là metaheuristic có khả năng thoát khỏi cực tiểu cục bộ. Trong Thí nghiệm 7, hệ thống xử lý một điểm xuất phát và chín điểm giao bằng hàm mục tiêu `balanced` tại khung giờ 07:30, với hành trình hở. Held–Karp giảm chi phí từ 4.320,1 xuống 2.494,9 giây quy đổi; NN + 2-opt/Or-opt đạt 2.534,2 giây; nghiệm tốt nhất trong năm lần chạy SA đạt 2.494,9 giây. Đây là các kết quả quan sát trên một cấu hình thí nghiệm cụ thể; chúng không tạo ra bảo đảm chất lượng tổng quát cho hai phương pháp heuristic.
...
Nhân viên giao hàng xuất phát tại **BT** (Chợ Bến Thành), giao tại **HN** (Điểm trung chuyển Hàm Nghi), **MT** (Bảo tàng Mỹ thuật TP.HCM) và **SC** (Saigon Centre/Takashimaya), không quay về BT. Bảng 8.4 là ma trận `balanced` tại khung giờ 07:30; mỗi ô là chi phí UCS giữa hai điểm và đã được làm tròn đến giây [P5].
...
*Hình 8.2. Đồ thị chi phí có hướng của ví dụ bốn điểm. Nhãn trên mỗi cung là chi phí `balanced` đã làm tròn, đơn vị giây quy đổi. Ba cung nét đậm tạo thành hành trình tối ưu `BT → SC → HN → MT`.*

Tính bất đối xứng thể hiện trực tiếp qua $c_{\mathrm{BT,SC}}=304$ nhưng $c_{\mathrm{SC,BT}}=99$. Bảng 8.4 và Hình 8.2 cung cấp cùng một bộ dữ liệu đầu vào và được sử dụng xuyên suốt ba ví dụ thuật toán sau:
...
Với ba điểm giao, có đúng $3!=6$ thứ tự cần xét. Bảng 8.6 liệt kê toàn bộ không gian nghiệm của ví dụ sau khi cố định BT ở vị trí đầu; vì vậy, kết luận tối ưu có thể được kiểm tra trực tiếp bằng phép cộng trên ma trận.

*Bảng 8.6. Toàn bộ sáu hành trình hở của ví dụ bốn điểm.*
...
### 8.2.3. Simulated Annealing — metaheuristic với nhiều hạt giống ngẫu nhiên
...
*Bảng 8.7. Cấu hình Simulated Annealing trong dự án.*
...
Bảng 8.8 chỉ so sánh các bộ giải trên ma trận $C$ đã dựng; chi phí UCS chung đã được tách ở phần luồng chương trình.

*Bảng 8.8. So sánh lý thuyết và phạm vi bảo đảm của ba phương pháp.*
...
Trong Bảng 8.8, $L$ là số lượt tìm kiếm cục bộ, $S$ là số hạt giống ngẫu nhiên và $I$ là số vòng lặp trên mỗi hạt giống; cấu hình hiện hành có $S=5$, $I=2.000$. “Trả nghiệm hợp lệ” chỉ nói rằng bộ giải tạo đủ thứ tự ghé theo hợp đồng, không đồng nghĩa với tìm được hành trình tốt nhất.
...
*Bảng 8.9. Thiết lập của thí nghiệm so sánh thứ tự ghé.*
...
*Bảng 8.10. So sánh kết quả cuối cùng của ba phương pháp trong Thí nghiệm 7.*
...
Các số trong Bảng 8.10 được làm tròn để trình bày. Từ số liệu chưa làm tròn, NN cao hơn Held–Karp khoảng 39,3 giây, tương đương 1,58%. Thống kê giữa năm lần chạy SA được trình bày riêng vì đây là một đại lượng tổng hợp, không phải “phương pháp thứ tư”: trung bình chi phí tốt nhất là $2.584{,}6\pm66{,}0$ giây quy đổi, cao hơn Held–Karp trung bình khoảng 89,8 giây, tương đương 3,60%. Mốc 40,5 ms bao gồm toàn bộ năm hạt giống, không phải một lần chạy đơn lẻ [P4].

### 8.3.3. Nỗ lực tìm kiếm theo từng phương pháp

Ba bộ giải không cùng dùng khái niệm “nút được mở rộng”: Held–Karp giải trạng thái quy hoạch động, tìm kiếm cục bộ đánh giá hành trình ứng viên, còn SA lấy mẫu các nước đi giả ngẫu nhiên. Vì vậy, Bảng 8.11 ghi các bộ đếm do phiên bản cài đặt [P1] xuất ra trên cùng đầu vào Thí nghiệm 7 [P4], theo đúng đơn vị của từng phương pháp thay vì gộp chúng thành một chỉ số không đồng nhất.

*Bảng 8.11. Nỗ lực tìm kiếm ghi nhận trên đầu vào Thí nghiệm 7.*
...
Các đơn vị trong Bảng 8.11 không tương đương về chi phí CPU, nên không thể so sánh trực tiếp chỉ bằng số đếm. Chúng chỉ giải thích hành vi: tìm kiếm cục bộ cải thiện hữu hạn quanh hành trình ban đầu; SA chủ động nhận 415 bước xấu để khám phá; Held–Karp bao phủ không gian trạng thái một cách có hệ thống.
...
- **Held–Karp; đồng thời là nghiệm tốt nhất trong năm hạt giống của SA:** Bưu điện Thành phố → Bitexco Financial Tower → BV Từ Dũ → Phố đi bộ Bùi Viện → Chợ Bến Thành → Nhà thờ Đức Bà → Thảo Cầm Viên → Công viên Lê Văn Tám → Chợ Tân Định → Chùa Vĩnh Nghiêm.
...
Hình 8.3 biểu diễn nghiệm Held–Karp trong cùng kịch bản; hướng di chuyển được đọc theo thứ tự các nhãn 1–9 trên tuyến.

![Hành trình Held–Karp trong Thí nghiệm 7](../results/figs/exp7_tsp_map.png)

*Hình 8.3. Hành trình Held–Karp trên `G_demo` với cấu hình 07:30. “Đi” là Bưu điện Thành phố; các nhãn 1–9 là thứ tự ghé chín điểm giao. Dòng “41,6 phút” là 2.494,9 giây `balanced` đổi sang phút quy đổi, không phải thời gian hành trình đo ngoài thực địa. Nguồn: Thí nghiệm 7 của dự án [P4].*
...
### 8.4.1. Tiêu chí phân loại nghiệm tối ưu và nghiệm gần đúng

Theo yêu cầu “*whether the result is optimal or approximate*” của đề bài, báo cáo phân biệt hai tầng kết luận. Thứ nhất, **bảo đảm của phương pháp** cho biết thuật toán có luôn trả nghiệm tối ưu khi các tiền đề được thỏa mãn hay không. Thứ hai, **chất lượng của nghiệm quan sát** cho biết nghiệm cụ thể có trùng với chuẩn chính xác trong thí nghiệm hay không. Hai tầng này không được đồng nhất: một heuristic có thể tình cờ tìm đúng nghiệm tối ưu ở một trường hợp nhưng vẫn không có bảo đảm tối ưu trên đầu vào khác.
...
*Bảng 8.12. Phân loại tính tối ưu của kết quả trong Thí nghiệm 7.*
...
Do đó, câu trả lời trực tiếp cho yêu cầu của đề bài là: kết quả Held–Karp **tối ưu theo bảo đảm thuật toán**; kết quả NN + 2-opt/Or-opt **gần đúng và cao hơn tối ưu 1,58%** trong trường hợp đã đo; kết quả tốt nhất của SA **trùng nghiệm tối ưu theo đối chứng hậu nghiệm**, nhưng không thể từ đó kết luận SA luôn tối ưu. Phạm vi “tối ưu” ở đây chỉ thuộc mô hình đã xác định—đồ thị, hồ sơ giao thông, hàm `balanced`, khung giờ 07:30 và hành trình hở—không phải tối ưu tuyệt đối đối với giao thông thực tế ngoài hiện trường.

### 8.4.2. Kiểm thử và khả năng tái lập

Các kiểm thử trong [P3] bao phủ những thuộc tính trực tiếp liên quan đến báo cáo.

*Bảng 8.13. Các nhóm kiểm thử ATSP và thuộc tính được xác minh.*
...
### 8.4.3. Hạn chế và nguy cơ đối với tính hợp lệ
...
### 8.4.4. Khi nào sử dụng phương pháp nào?

*Bảng 8.14. Khuyến nghị lựa chọn phương pháp theo nhu cầu sử dụng.*
...
*Bảng 8.15. Lộ trình cải tiến ATSP và bằng chứng cần đạt.*
...
### 8.4.6. Kết luận phần tối ưu đa điểm
